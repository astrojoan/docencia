...
# -------------------------
# Càlcul de G i incertesa
# -------------------------
def beta_from_geom(b, d):
    """beta = b^3 / (b^2 + 4 d^2)^(3/2)"""
    return b**3 / (b**2 + 4.0 * d**2) ** 1.5


def phi1_from_mb(mb, m2, r2, d):
    """
    Phi1 = 1 + (mb/m2) / [6 * (1 + (2/5)*(r2/d)^2)]
    """
    return 1.0 + (mb / m2) / (6.0 * (1.0 + (2.0 / 5.0) * (r2 / d) ** 2))


def phi2_from_alpha_T(alpha, T):
    """
    Phi2 = 1 + alpha^2 T^2 / (16 pi^2)
    """
    return 1.0 + (alpha**2 * T**2) / (16.0 * np.pi**2)


def compute_G_from_params(
    Sl, alpha, T,
    # constants experimentals (SI)
    L, m1, m2, d, b, r2, mb,
):
    """
    Retorna:
      G, theta_l, beta, phi1, phi2

    IMPORTANT:
    - Sl i L han d'estar en les mateixes unitats.
    - Ací tot va en SI (m, kg, s).
    - Per a theta_l usem |Sl|, perquè físicament és una amplitud.
    """
    theta_l = abs(Sl) / (2.0 * L)
    beta = beta_from_geom(b, d)
    phi1 = phi1_from_mb(mb, m2, r2, d)
    phi2 = phi2_from_alpha_T(alpha, T)

    geom = (d**2 + (2.0 / 5.0) * r2**2) * b**2
    G = (theta_l / T**2) * (4.0 * np.pi**2) * geom / (m1 * d * (1.0 - beta)) * phi1 * phi2

    return G, theta_l, beta, phi1, phi2


def compute_G_mc(
    popt, pcov,
    # unitat de les dades de posició del fit:
    # 1e-2 si y està en cm, 1e-3 si y està en mm
    pos_unit_to_m=1e-2,
    # constants experimentals (valor, error) en SI
    L=3.144,  sL=0.005,
    m1=1.500, sm1=0.0,
    m2=0.0383, sm2=0.0002,
    d=0.0500, sd=0.0001,
    b=0.0422, sb=0.0001,
    r2=0.00955, sr2=0.00001,
    mb=0.0050, smb=0.0030,
    n_draws=20000,
    seed=12345,
):
    """
    Calcula G i la seua incertesa per Monte Carlo, aprofitant:
    - correlacions entre Sl, alpha, T del fit (via pcov)
    - incerteses independents de les constants instrumentals

    Retorna un diccionari amb:
      G, sG, theta_l, stheta_l, beta, sbeta, phi1, sphi1, phi2, sphi2
    """
    rng = np.random.default_rng(seed)

    # fem la covariància PSD per seguretat numèrica
    pcov_psd = make_psd(np.array(pcov, dtype=float))

    draws = rng.multivariate_normal(mean=np.array(popt, float), cov=pcov_psd, size=n_draws)

    # Paràmetres del fit
    Sl_draw = draws[:, 1] * pos_unit_to_m   # de "unitats del regle" a metres
    alpha_draw = draws[:, 2]
    T_draw = draws[:, 3]

    ok = np.isfinite(Sl_draw) & np.isfinite(alpha_draw) & np.isfinite(T_draw) & (T_draw > 0)
    Sl_draw = Sl_draw[ok]
    alpha_draw = alpha_draw[ok]
    T_draw = T_draw[ok]

    n = len(Sl_draw)
    if n == 0:
        raise RuntimeError("No hi ha mostres vàlides per al càlcul Monte Carlo de G.")

    # Constants instrumentals
    def sample_pos(mu, sigma, size):
        if sigma <= 0:
            return np.full(size, mu, dtype=float)
        return rng.normal(mu, sigma, size)

    L_draw  = sample_pos(L,  sL,  n)
    m1_draw = sample_pos(m1, sm1, n)
    m2_draw = sample_pos(m2, sm2, n)
    d_draw  = sample_pos(d,  sd,  n)
    b_draw  = sample_pos(b,  sb,  n)
    r2_draw = sample_pos(r2, sr2, n)
    mb_draw = sample_pos(mb, smb, n)

    # filtres físics mínims
    ok2 = (
        (L_draw > 0) & (m1_draw > 0) & (m2_draw > 0) &
        (d_draw > 0) & (b_draw > 0) & (r2_draw > 0) & (mb_draw >= 0)
    )

    Sl_draw = Sl_draw[ok2]
    alpha_draw = alpha_draw[ok2]
    T_draw = T_draw[ok2]
    L_draw = L_draw[ok2]
    m1_draw = m1_draw[ok2]
    m2_draw = m2_draw[ok2]
    ...
    theta_draw = np.abs(Sl_draw) / (2.0 * L_draw)
    beta_draw = beta_from_geom(b_draw, d_draw)
    phi1_draw = phi1_from_mb(mb_draw, m2_draw, r2_draw, d_draw)
    phi2_draw = phi2_from_alpha_T(alpha_draw, T_draw)

    geom_draw = (d_draw**2 + (2.0 / 5.0) * r2_draw**2) * b_draw**2

    G_draw = (
        (theta_draw / T_draw**2)
        * (4.0 * np.pi**2)
        * geom_draw
        / (m1_draw * d_draw * (1.0 - beta_draw))
        * phi1_draw
        * phi2_draw
    )
    ...
    # resum 1 sigma
    out = {
        "G": float(np.mean(G_draw)),
        "sG": float(np.std(G_draw, ddof=1)),
        "theta_l": float(np.mean(theta_draw)),
        "stheta_l": float(np.std(theta_draw, ddof=1)),
        "beta": float(np.mean(beta_draw)),
        "sbeta": float(np.std(beta_draw, ddof=1)),
        "phi1": float(np.mean(phi1_draw)),
        "sphi1": float(np.std(phi1_draw, ddof=1)),
        "phi2": float(np.mean(phi2_draw)),
        "sphi2": float(np.std(phi2_draw, ddof=1)),
        "n_ok": int(len(G_draw)),
    }
    return out
...
def main():
    ...
    # Càlcul de G
    # -------------------------
    # Tria ací la conversió correcta:
    #   1e-2 -> si les teues y/Sl estan en cm
    #   1e-3 -> si les teues y/Sl estan en mm
    POS_UNIT_TO_M = 1e-2

    g_res = compute_G_mc(
        popt, pcov,
        pos_unit_to_m=POS_UNIT_TO_M,
        # valors del vostre muntatge (SI)
        L=3.144,   sL=0.005,      # 314.4 ± 0.5 cm
        m1=1.500,  sm1=0.0,       # 1500 g
        m2=0.0383, sm2=0.0002,    # 38.3 ± 0.2 g
        d=0.0500,  sd=0.0001,     # 5.00 ± 0.01 cm
        b=0.0422,  sb=0.0001,     # 4.22 ± 0.01 cm
        r2=0.00955, sr2=0.00001,  # 0.955 ± 0.001 cm
        mb=0.0050, smb=0.0030,    # 5 ± 3 g (si uses la mitjana 2–8 g)
        n_draws=20000,
        seed=12345,
    )

    print("\nResultats derivats")
    print(f"theta_l = {g_res['theta_l']:.6g} ± {g_res['stheta_l']:.2g} rad")
    print(f"beta    = {g_res['beta']:.6g} ± {g_res['sbeta']:.2g}")
    print(f"phi1    = {g_res['phi1']:.6g} ± {g_res['sphi1']:.2g}")
    print(f"phi2    = {g_res['phi2']:.6g} ± {g_res['sphi2']:.2g}")
    print(f"G       = {g_res['G']:.6g} ± {g_res['sG']:.2g} N m^2 kg^-2")
